...
public class ValuationController {
    ...
    public Map valuation(@RequestBody ValuationParam valuationParam) throws NotFoundException, MissingParameterException {
        if (ObjectUtils.isEmpty(valuationParam)) {
            throw new MissingParameterException("valuation.item");
        }
        if (StringUtils.isEmpty(valuationParam.getModelId())) {
            throw new MissingParameterException("modelId.item");
        }
        if (StringUtils.isEmpty(valuationParam.getRegDate())) {
            throw new MissingParameterException("regDate.item");
        }
        if (StringUtils.isEmpty(valuationParam.getMile())) {
            throw new MissingParameterException("mile.item");
        }
        if (StringUtils.isEmpty(valuationParam.getZone())) {
            throw new MissingParameterException("zone.item");
        }
