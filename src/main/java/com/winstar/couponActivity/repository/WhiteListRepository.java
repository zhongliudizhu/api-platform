...
public interface WhiteListRepository extends JpaRepository<WhiteList,String>,JpaSpecificationExecutor<WhiteList> {
    ...
    @Query(value = "select card_number from cbc_white_list where SUBSTR(driver_license,-6)= ?1 and phone_number=?2 and type=?3",nativeQuery = true)
    String findByDriverLicenseAndPhoneNumber(String driverLicense,String phone,Integer type);
